...
export const blockedClientRequestsTotal = new Counter({
  name: 'rateforge_blocked_client_requests_total',
  help: 'Total blocked requests grouped by client IP for dashboard inspection.',
  labelNames: ['service', 'client_ip'] as const,
  registers: [metricsRegistry],
});
...
export function recordBlockedRequest(
  ...
  blockedClientRequestsTotal.inc({
    service: SERVICE_NAME,
    client_ip: req.clientIdentity?.ip ?? req.ip ?? req.socket?.remoteAddress ?? 'unknown',
  });
}
